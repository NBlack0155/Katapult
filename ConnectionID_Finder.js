(() => {
async function findJobIdByConnectionId() {
  const connectionId = prompt("Enter Connection ID:");
  if (!connectionId) return;

  try {
    const snap = await FirebaseWorker
      .ref('photoheight/job_permissions/-Os2Y1uLFmB9CqVnQG3c/list')
      .once('value');
    ...
    for (let i = 0; i < jobIds.length; i += batchSize) {
      ...
      const results = await Promise.all(
        batch.map(jobId =>
          FirebaseWorker
            .ref(`photoheight/jobs/${jobId}/connections/${connectionId}`)
            .once('value')
      ...
      for (const r of results) {
        ...
        if (r.exists) {
          ...
          const url = `https://ecc.katapultpro.com/map/#${r.jobId}/c${connectionId}`;
    ...
    console.log("No match found for connectionId:", connectionId);
